    ...
    <el-card class="search-card">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-select v-model="sortState.sortBy" style="width:160px" @change="loadTable">
            <el-option value="createTime" label="下单时间" />
            <el-option value="modifyTime" label="最后更新时间" />
          </el-select>
          <el-select v-model="sortState.sortOrder" style="width:120px;margin-left:8px" @change="loadTable">
      ...
      <el-table :data="tableData" border v-loading="loading" style="width:100%"
        ...
        <el-table-column label="订单状态" width="110" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.hasRefundRequest" type="warning" size="small">待退款</el-tag>
            <el-tag v-else-if="row.orderStatus === 1" type="info" size="small">待配送</el-tag>
            <el-tag v-else :type="statusType(row.orderStatus)" size="small">{{ statusLabel(row.orderStatus) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-tabs v-model="activeTab">
          ...
          <el-tab-pane label="商品信息" name="products">
            <el-table v-if="flatSkuList.length > 0" :data="flatSkuList" border size="small" style="width:100%">
              ...
              <el-table-column label="规格" min-width="130"><template #default="{ row: sku }">{{ sku.attrsText || '-' }}</template></el-table-column>
              <el-table-column label="原价" width="85"><template #default="{ row: sku }"><span style="color:#c0c4cc;text-decoration:line-through;font-size:12px">¥{{ sku.oldPrice }}</span></template></el-table-column>
              <el-table-column label="现价" width="80"><template #default="{ row: sku }"><span style="color:#f56c6c">¥{{ sku.price }}</span></template></el-table-column>
              <el-table-column label="数量" width="60" prop="quantity" align="center" />
              <el-table-column label="总价" width="85"><template #default="{ row: sku }"><span style="font-weight:600">¥{{ (sku.price * sku.quantity).toFixed(2) }}</span></template></el-table-column>
            </el-table>
...
async function loadTable() {
  ...
  try {
    ...
    if (data) { tableData.value = data.items || data || []; pagination.counts = data.counts || 0; pagination.pages = data.pages || 0 }
  } catch { /* ignore */ }
...
async function handleConfirmReceipt(row: any) {
  try {
    ...
    await confirmReceipt(row.id); ElMessage.success('已确认收货')
    row.orderStatus = 3; loadTable()
  } catch { /* ignore */ }
}

async function handleDispatch(row: any) {
  try {
    await ElMessageBox.confirm('确认配送？', '提示', { type: 'warning' })
    await dispatchOrder(row.id); ElMessage.success('发货成功')
    row.orderStatus = 2; loadTable()
  } catch { /* ignore */ }
}
async function handleApproveRefund(row: any) {
  try {
    await ElMessageBox.confirm('确认同意退款？', '提示', { type: 'warning' })
    await approveRefund(row.id); ElMessage.success('已同意退款')
    row.orderStatus = 5; row.hasRefundRequest = false; loadTable()
  } catch { /* ignore */ }
}
async function handleRejectRefund(row: any) {
  try {
    await ElMessageBox.confirm('确认拒绝退款？', '提示', { type: 'warning' })
    await rejectRefund(row.id); ElMessage.success('已拒绝退款')
    row.hasRefundRequest = false; loadTable()
  } catch { /* ignore */ }
}
async function handleCancel(row: any) {
  try {
    await ElMessageBox.confirm('确认取消该订单？', '提示', { type: 'warning' })
    await cancelOrder(row.id); ElMessage.success('已取消订单')
    row.orderStatus = 5; loadTable()
  } catch { /* ignore */ }
}
...
.pending-order { display: flex; flex-direction: column; gap: 16px; max-width: 1400px; margin: 0 auto; }
.search-card { margin-bottom: 0; }
.page-subtitle { font-size: 16px; font-weight: 600; color: #303133; margin: 0 0 12px 0; }
